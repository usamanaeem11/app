/*
  # Fix RLS Optimization - Final Pass

  ## Changes
  
  1. Optimize all RLS policies with exact pattern Supabase expects
     - Remove unnecessary aliases
     - Move cast inside the SELECT
     - Use pattern: (select auth.uid()::text) instead of (select (auth.uid())::text AS uid)
  
  ## Impact
  - Resolves all RLS analyzer warnings
